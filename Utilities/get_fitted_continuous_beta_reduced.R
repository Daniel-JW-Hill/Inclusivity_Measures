...
get_fitted_continuous_reduced = function(hh_data, 
  ...
  results_hh_model_reduced = data.frame("coefs" = c(model_summary_hh_reduced$coefficients[[1]][,1], model_summary_hh_reduced$coefficients[[2]][,1]),
                                     "standard_errors" = c(model_summary_hh_reduced$coefficients[[1]][,2], model_summary_hh_reduced$coefficients[[2]][,2]),
                                     "probs" = c(model_summary_hh_reduced$coefficients[[1]][,4], model_summary_hh_reduced$coefficients[[2]][,4]))
  
  write.csv(results_hh_model_reduced, here("results", "Continuous Household model - reduced - results.csv"))
  ...
  pred_opportunity_zprob= predict(model_hh_reduced, newdata = hh_data_pbar, type = 'zprob', se.fit = TRUE)
  pred_opportunity_response = predict(model_hh_reduced, newdata = hh_data_pbar, type = 'response', se.fit = TRUE)
  pred_opportunity_conditional = predict(model_hh_reduced, newdata = hh_data_pbar, type = 'conditional', se.fit = TRUE)
  
  #Preference fitted values
  pred_preferences_zprob= predict(model_hh_reduced, newdata = hh_data_cbar, type = 'zprob', se.fit = TRUE)
  pred_preferences_response = predict(model_hh_reduced, newdata = hh_data_cbar, type = 'response', se.fit = TRUE)
  pred_preferences_conditional = predict(model_hh_reduced, newdata = hh_data_cbar, type = 'conditional', se.fit = TRUE)
  
  #Fitted outcomes (without error term)
  pred_zprob = predict(model_hh_reduced, newdata = hh_data, type = 'zprob', se.fit = TRUE)
  pred_response = predict(model_hh_reduced, newdata = hh_data, type = 'response', se.fit = TRUE)
  pred_conditional = predict(model_hh_reduced, newdata = hh_data, type = 'conditional', se.fit = TRUE)
  ...
  for (bb in 1:b){
    ...
    #re-estimate model
    model_hh_reduced_boot = glmmTMB(eq_hh_reduced, data = bootstrap_sample, family = beta_family(link = "logit"), ziformula = eq_hh_zero, dispformula = ~ 1)
    
    #Check if converged, if no add central scenario
    if (model_hh_reduced_boot$fit$convergence !=0){
    ...
    fitted_opportunity_hh_model_zprob_dist[,bb] = predict(model_hh_reduced_boot, newdata = hh_data_pbar, type = 'zprob')
    fitted_opportunity_hh_model_response_dist[,bb] = predict(model_hh_reduced_boot, newdata = hh_data_pbar, type = 'response')
    fitted_opportunity_hh_model_conditional_dist[,bb] = predict(model_hh_reduced_boot, newdata = hh_data_pbar, type = 'conditional')
    fitted_preferences_hh_model_zprob_dist[,bb] = predict(model_hh_reduced_boot, newdata = hh_data_cbar, type = 'zprob')
    fitted_preferences_hh_model_response_dist[,bb] = predict(model_hh_reduced_boot, newdata = hh_data_cbar, type = 'response')
    fitted_preferences_hh_model_conditional_dist[,bb] = predict(model_hh_reduced_boot, newdata = hh_data_cbar, type = 'conditional')
    fitted_hh_model_zprob_dist[,bb] = predict(model_hh_reduced_boot, newdata = hh_data, type = 'zprob')
    fitted_hh_model_response_dist[,bb] = predict(model_hh_reduced_boot, newdata = hh_data, type = 'response')
    fitted_hh_model_conditional_dist[,bb] = predict(model_hh_reduced_boot, newdata = hh_data, type = 'conditional')
    
    # Finally retrieve central estimate only for the significant coefficients. 
    counter = 1
    
    if (any(is.na(summary(model_hh_reduced)$coefficients$cond[,2]))) {
      ...
      fitted_opportunity_hh_model_zprob_dist_onlysig[,bb] = predict(model_hh_reduced, newdata = hh_data_pbar, type = 'zprob')
      fitted_opportunity_hh_model_response_dist_onlysig[,bb] = predict(model_hh_reduced, newdata = hh_data_pbar, type = 'response')
      fitted_opportunity_hh_model_conditional_dist_onlysig[,bb] = predict(model_hh_reduced, newdata = hh_data_pbar, type = 'conditional')
        ...
        fitted_opportunity_hh_model_zprob_dist_onlysig[,bb] = predict(model_hh_reduced_boot, newdata = hh_data_pbar, type = 'zprob')
        fitted_opportunity_hh_model_response_dist_onlysig[,bb] = predict(model_hh_reduced_boot, newdata = hh_data_pbar, type = 'response')
        fitted_opportunity_hh_model_conditional_dist_onlysig[,bb] = predict(model_hh_reduced_boot, newdata = hh_data_pbar, type = 'conditional')
      }
  }
  # Return outputs
