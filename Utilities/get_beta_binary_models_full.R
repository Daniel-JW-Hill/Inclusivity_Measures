...
get_fitted_binary_full = function(hh_data, 
  ...
  write.csv(tests_df, here("results", "Binary  Household model - full - results.csv"))
  ...
  write.csv(tests_df, here("results", "Binary individual model - full - results.csv"))
  ...
  for (bb in 1:b){
    #Draw sample of coefficients from coef and sd 
    results_indiv_model_full$new_coefs = rnorm(n = length(results_indiv_model_full$coefs), mean = results_indiv_model_full$coefs, sd = results_indiv_model_full$standard_errors)
