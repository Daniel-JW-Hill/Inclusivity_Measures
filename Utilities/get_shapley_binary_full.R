...
# This function performs the decompositions for the full hh and individual models. 
# 

get_contributions_binary_full = function(hh_data, 
                                    indiv_data,
                                    Outcome_hh,
                                    C_hh,
                                    P_hh,
  ...
  fitted_opportunity =  predict(model_hh_full, newdata = hh_data_pbar, type = 'response' )
  fitted_preferences = predict(model_hh_full, newdata = hh_data_cbar, type = 'response' )
  fitted_outcomes = predict(model_hh_full, data = hh_data, type = 'response')
  ...
  for (i in 1:length(C_hh)) {
    ...
    pred_opportunity_c = predict(model_hh_full, newdata = hh_data_new, type = 'response')
  ...
  write.csv(opportunity_decomp_hh, file = here("Results", "Opportunity decomposition binary HH full model.csv"))
  ...
  #Estimate central model
  eq_indiv_full =  paste(Outcome_indiv, sep = " ~ ", paste(paste(C_indiv, collapse = " + "), paste(P_indiv, collapse = " + "), Z_indiv, sep = " + ")) #Estimate model
