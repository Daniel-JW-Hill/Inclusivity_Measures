...
get_contributions_continuous = function(hh_data, 
  ...
  for (i in 1:length(C_hh)) {
    ...
    pred_opportunity_zi_c = predict(model_hh_full, newdata = hh_data_new, type = 'zprob')
    pred_opportunity_response_c = predict(model_hh_full, newdata = hh_data_new, type = 'response')
    pred_opportunity_conditional_c = predict(model_hh_full, newdata = hh_data_new, type = 'conditional')
  ...
  write.csv(opportunity_decomp_hh_zi, file = here("Results", "Opportunity decomposition continuous zi full model.csv"))
  write.csv(opportunity_decomp_hh_res, file = here("Results", "Opportunity decomposition continuous response full model.csv"))
  write.csv(opportunity_decomp_hh_cond, file = here("Results", "Opportunity decomposition continuous conditional full model.csv"))
